...
class Interface
  ...
  def attach_carriages
    train = train_selector
    if train.type == 'cargo'
      train.attach_carriage CargoCarriage.new(train.vendor)
    else
      train.attach_carriage PassengerCarriage.new(train.vendor)
    end
  ...
  def validate_user_input
    ...
    puts "Wrong input. Read requirments and retry input.\nError: #{e.message}"
    retry
  end
end
